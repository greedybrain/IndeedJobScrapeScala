...
import com.wycbw_apps.CLI.{getDatePosted, getDesiredSalary, getExpLevel, getJobTitle, getJobType, getLocation, getRadius}
...
class URLBuild(
  ...
  def thenBuildURL: String =
    "https://www.indeed.com/jobs?q=" + jobTitle +
      "$" + desiredSalary +
      "&l=" + location +
      "&radius=" + radius +
      "&jt=" + jobType +
...
object URLBuild {
  ...
  def getURL: String = {
    val getObjectParams = new URLBuild(getJobTitle, getDesiredSalary, getLocation, getRadius, getJobType, getExpLevel, getDatePosted)
